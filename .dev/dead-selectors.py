...
REGISTERED = re.compile(r"""array\(\s*'(unioncorp-[a-z0-9-]+)'\s*,\s*__\(""")
...
def main():
    ...
    # theme shipped — `unioncorp-card` on 30 groups, `unioncorp-ghost` on 10
    # buttons, `unioncorp-ticks` on 10 lists, and not one rule for any of them.
