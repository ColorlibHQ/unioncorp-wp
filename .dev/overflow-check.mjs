 ...
 *
 *   WP_URL=https://example.com UNIONCORP_PATHS=/,/contact/ node .dev/overflow-check.mjs
 *
 * @package Unioncorp
 */
...
const paths = ( process.env.UNIONCORP_PATHS || '/' ).split( ',' ).filter( Boolean );
const widths = ( process.env.UNIONCORP_WIDTHS || '1400,1024,768,390' ).split( ',' ).map( Number );
