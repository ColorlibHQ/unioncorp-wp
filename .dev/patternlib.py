#!/usr/bin/env python3
"""Block markup helpers for Unioncorp's pattern generators.
...
def separator(style="unioncorp-diamond", align=None):
    data = {"className": "is-style-" + style}
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    url = "<?php echo esc_url( get_theme_file_uri( 'assets/images/%s.avif' ) ); ?>" % slug
    attr_json = attrs(data).replace('"UNIONCORP_IMAGE"', '"%s"' % url)
